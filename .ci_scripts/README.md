...
- [`validate_agents.py`](validate_agents.py) - checks that every agent file under `agents/` has well-formed frontmatter, a `name` matching its filename, a `model`, `permissionMode`, `memory`, `isolation`, and `maxTurns` that Claude Code accepts, tool lists without empty entries, preloaded `skills` that exist under `skills/`, an H1 body opening with instructions beneath it, no HTML comments, and an entry in the agent index.
